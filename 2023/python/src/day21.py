...
def part2(input):
    ...
    middle_left = bfs(input, (middle, w-1), 130, 1)
    middle_right = bfs(input, (middle, 0), 130, 1)
    middle_down = bfs(input, (0, middle), 130, 1)
    middle_up = bfs(input, (middle, w-1), 130, 1)
    ...
    return ret, bfs2(input, start, steps, steps % 2)
...
def bfs2(map, start, steps, m):
    ...
    while i < steps:
        ...
        for (r,c) in current:
            neigh = [(r+1, c),(r-1, c), (r, c+1),(r, c-1)]
            for n in neigh:
                if n not in visited and map[n[0]%len(map)][n[1]%len(map)] != "#":
                    if (i+1) % 2 == m:
                        count += 1
                    next.add(n)
                    visited.add(n)
